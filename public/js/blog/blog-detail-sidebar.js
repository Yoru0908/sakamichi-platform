...
window.BlogDetailSidebar = {
  ...
  async loadMemberAvatar(blog) {
    ...
    try {
      ...
      // Use sessionStorage cache to avoid re-fetching on every detail view
      let data = null;
      const cacheKey = 'member_images_json';
      const cached = sessionStorage.getItem(cacheKey);
      if (cached) {
        try { data = JSON.parse(cached); } catch (_) { /* ignore parse errors */ }
      }
      if (!data) {
        const response = await fetch('/data/member-images.json');
        if (response.ok) {
          data = await response.json();
          try { sessionStorage.setItem(cacheKey, JSON.stringify(data)); } catch (_) { /* quota exceeded */ }
        }
      }
      if (data) {
  ...
  renderNewEntries(blogs) {
    ...
    entriesEl.innerHTML = blogs.map(blog => {
      ...
      // Optimize thumbnail with Cloudinary (100px for 80x80 display)
      let optimizedThumbUrl = thumbnailUrl;
      if (thumbnailUrl && typeof window.getCloudinaryUrl === 'function') {
        optimizedThumbUrl = window.getCloudinaryUrl(thumbnailUrl, 100);
      }

      // 如果没有找到图片,使用默认渐变背景
      const thumbnailStyle = optimizedThumbUrl
        ? `background-image: url('${optimizedThumbUrl}'); background-size: cover; background-position: center;`
