...
test('toolbar follows the active supported site in either language edition',async()=>{
 for(const [url,expected] of [['https://saka46log.com/results','https://saka46log.com/import'],['https://46log.com/miguri','https://46log.com/miguri'],['https://saka46log.com.evil.invalid/import','https://46log.com/miguri']]){
  const h=harness();await h.click({url});assert.equal(h.created[0].url,expected);assert.equal(h.updated.length,0);
 }
});
