...
export type FortuneMeetsMemberSummary = {
  name: string;
  closedCount: number;
  totalCount: number;
};

export type FortuneMeetsAwardAnalysis = {
  awardId: number;
  mode: FortuneMeetsMode;
  name: string;
  title: string;
  group: FortuneMeetsGroupId;
  groupName: string;
  sourceUrl: string;
...
export type FortuneMeetsConfigAnalysis = {
  eventId: string;
  eventName: string;
  artistName: string;
  group: FortuneMeetsGroupId;
...
export function normalizeFortuneMeetsMember(name: string): string {
...
function inferGroup(config: RawConfig, source?: Partial<FortuneMeetsSource>): FortuneMeetsGroupId {
  if (source?.artist && GROUP_BY_ARTIST[source.artist]) return GROUP_BY_ARTIST[source.artist];
  const artistName = `${config.artistName || ''}`;
  return GROUP_BY_NAME[artistName] || 'sakurazaka';
}
...
function analyzeAward({
  ...
  mode: FortuneMeetsMode;
  config: RawConfig;
  source: FortuneMeetsSource;
  now: string;
}): FortuneMeetsAwardAnalysis {
  ...
  const sourceConfigUrl = configUrl(source);
  const sourceUrl = pageUrl(source);
...
export function analyzeFortuneMeetsConfig(
  config: RawConfig,
  source: FortuneMeetsSource,
  options: { now?: string } = {},
): FortuneMeetsConfigAnalysis {
...
export async function fetchFortuneMeetsAnalysis(
  source: FortuneMeetsSource,
  fetchImpl: typeof fetch = fetch,
): Promise<FortuneMeetsConfigAnalysis> {
  ...
  if (!res.ok) {
    throw new Error(`Fortune Meets config fetch failed: ${res.status}`);
  }
