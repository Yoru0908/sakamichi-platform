...
export function isNavItemActive(item: NavItem, path: string): boolean {
  return (item.href === '/' ? path === '/' : path === item.href || path.startsWith(item.href + '/'))
    || !!item.children?.some((child) => isNavItemActive(child, path));
}
...
export const NAV_ITEMS: NavItem[] = [
  ...
  { labelKey: 'nav.schedule', href: '/schedule', icon: 'CalendarDays' },
  {
    ...
    children: [
      ...
      { labelKey: 'nav.miguri.history', href: '/miguri/history', icon: 'Archive' },
...
export const FOOTER_COMMUNITY_LINKS: NavItem[] = [
  { labelKey: 'nav.miguri.history', href: '/miguri/history' },
